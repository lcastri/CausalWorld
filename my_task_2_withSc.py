...
def brightness(z, max_height, v, blending_factor = 0.5):
    brightness_depth = 1 - ((max_height - abs(z)) / max_height)
    brightness_motion = 1 - (v / MAX_VEL)

    final_brightness = np.clip(blending_factor * brightness_depth + (1 - blending_factor) * brightness_motion, 0, 1)
...
def change_floor_colour(env, z):
    R, G, B = FLOOR_COLOUR
    b = brightness(z, MAX_HEIGHT, 0, 0.66)*0.3 + 0.6
    R = np.clip(b*R, 0, 1)
    G = np.clip(b*G, 0, 1)
    B = np.clip(b*B, 0, 1)
...
def change_box_colour(env, z, v, s):
    R, G, B = BOX_COLOUR
    b = brightness(z - 0.095, MAX_HEIGHT - 0.095, v, 0.25)
    # R = np.clip(b*R, 0, 1)
    # G = np.clip(b*G, 0, 1)
    # B = np.clip(b*B, 0, 1)
    R = np.clip(b*R + 0.075*s, 0, 1)
    G = np.clip(b*G + 0.075*s, 0, 1)
    B = np.clip(b*B + 0.075*s, 0, 1)
    colour = np.array([R, G, B])
...
def init(intervention = False):
    ...
    if not intervention:
        Bc = change_box_colour(env, robot.pos.z, robot.v, Sc)
    else:
    ...
    df.loc[0] = [Fc, robot.pos.z, Bc, Sc, robot.v]
    df.loc[1] = [Fc, robot.pos.z, Bc, Sc, robot.v]
...
def run(intervention = False):
    ...
    for t in range(2, T):
        ...
        Sc = change_stage_colour(env, df.loc[t]['S_c'])
        Fc = change_floor_colour(env, df.loc[t-2]['H'])
        if not intervention: 
            Bc = change_box_colour(env, df.loc[t-1]['H'], df.loc[t-1]['v'], df.loc[t-1]['S_c'])
        else:
            colour = np.array(BOX_INT_COLOUR)
            env.do_intervention({'tool_block' : {'color' : colour}})
            Bc = np.sum(colour)   
        ...
        if not intervention: 
            df.loc[t] += [Fc, F1.pos.z, Bc, Sc - df.loc[t]['S_c'], F1.v]
        else:
            df.loc[t] += [Fc, F1.pos.z, Bc - df.loc[t]["B_c"], Sc - df.loc[t]['S_c'], F1.v]
